...
function parseFrameHeader(header) {
  let b1, b2, versionBits, version, simpleVersion, layerBits, layer, bitRateKey, bitRateIndex, bitRate, sampleRateIdx, sampleRate, paddingBit, sample;
...
async function handleMp3(filename, cbrEstimate) {
  let duration = 0,
    fd,
    buffer,
    bytesRead,
  ...
  try {
    ...
    while (offset < (isBuffer ? srcBuffer.length : stat.size)) {
      ...
      } else if (buffer[0] === 0x54 && buffer[1] === 0x41 && buffer[2] === 0x47) {
        //'TAG'
...
async function musicDuration(filename, fileType = '.mp3', cbrEstimate = false) {
  // let fileType = filename.substring(filename.lastIndexOf('.'), filename.length);

  // Only mp3 for the moment
  switch (fileType) {
    case '.mp3':
      return await handleMp3(filename, cbrEstimate);
      break;
    default:
      throw new Error(`fileType not supported: ${fileType}`);
  }
}
